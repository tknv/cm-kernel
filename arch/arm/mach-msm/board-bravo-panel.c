...
static struct msm_lcdc_panel_ops bravo_lcdc_amoled_panel_ops = {
	.init		= samsung_oled_panel_init,
	.blank		= samsung_oled_panel_blank,
	.unblank	= samsung_oled_panel_unblank,
};

static struct msm_lcdc_panel_ops bravo_lcdc_tft_panel_ops = {
...
static struct msm_lcdc_timing bravo_lcdc_amoled_timing = {
		.clk_rate		= 24576000,
		.hsync_pulse_width	= 4,
		.hsync_back_porch	= 8,
		.hsync_front_porch	= 8,
...
static struct msm_lcdc_timing bravo_lcdc_tft_timing = {
		.clk_rate		= 24576000,
		.hsync_pulse_width	= 2,
		.hsync_back_porch	= 20,
...
static struct msm_fb_data bravo_lcdc_fb_data = {
		.xres		= 480,
		.yres		= 800,
		.width		= 48,
		.height		= 80,
...
static struct msm_lcdc_platform_data bravo_lcdc_amoled_platform_data = {
	.panel_ops	= &bravo_lcdc_amoled_panel_ops,
	.timing		= &bravo_lcdc_amoled_timing,
	.fb_id		= 0,
	.fb_data	= &bravo_lcdc_fb_data,
	.fb_resource	= &resources_msm_fb[0],
};

static struct msm_lcdc_platform_data bravo_lcdc_tft_platform_data = {
	.panel_ops	= &bravo_lcdc_tft_panel_ops,
	.timing		= &bravo_lcdc_tft_timing,
	.fb_id		= 0,
	.fb_data	= &bravo_lcdc_fb_data,
	.fb_resource	= &resources_msm_fb[0],
};

static struct platform_device bravo_lcdc_amoled_device = {
	.name	= "msm_mdp_lcdc",
	.id	= -1,
	.dev	= {
		.platform_data = &bravo_lcdc_amoled_platform_data,
	},
};

static struct platform_device bravo_lcdc_tft_device = {
	.name	= "msm_mdp_lcdc",
	.id	= -1,
	.dev	= {
		.platform_data = &bravo_lcdc_tft_platform_data,
	},
};

static int bravo_init_spi_hack(void)
{
	int ret;
...
static void bravo_brightness_set(struct led_classdev *led_cdev,
...
static void bravo_brightness_amoled_set_work(struct work_struct *work_ptr)
{
	unsigned long flags;
	uint8_t val;
...
static struct led_classdev bravo_brightness_led = {
	.name = "lcd-backlight",
	.brightness = LED_FULL,
	.brightness_set = bravo_brightness_set,
};
...
device_initcall(bravo_init_panel);
